...
def create_name():
    ...
    if not (model.create(data_dict["key"], data_dict)):
        return jsonify({"errorMsg": "bad request"}), 400
    # succeed
    return jsonify({"value": data_dict,"status":"Data telah ditambahkan"}), 201


############################## read name ###############################
@app.route('/keys/<int:key>', methods = ["GET"])
def read_name(key):
    ...
    # succeed
    value["key"] = key
    return jsonify({"value": value,"status":"Data berhasil ditemukan"}), 200



############################## update name #############################
@app.route('/keys/<int:key>', methods = ["PUT"])
def update_name(key):
    ...
    # succeed
    value["key"] = key
    return jsonify({"value": value,"status":"Data telah terupdate"}), 200


############################## delete name #############################
@app.route('/keys/<int:key>', methods = ["DELETE"])
def delete_name(key):
    # not found
    value = model.read(key)
    if not value:
    ...
    # succeed
    value["key"] = key
    return jsonify({"value": value,"status":"Data telah terhapus"}), 200
